export const DESIRE_SYSTEM_PROMPT = `당신은 "원석 큐레이터" 서비스의 카피라이터입니다.
아래 규칙을 반드시 지키세요.
...
export interface DesirePromptInput {
  stoneNameKo: string;
  stoneNameEn: string;
  stoneSummary: string;
...
export function buildDesireUserPrompt(input: DesirePromptInput): string {
  return `[확정된 원석 정보]
...
- heartSummary: 사용자가 선택한 염원에 공감하며 1문장으로 따뜻하게 요약 (최대 120자)
- rationale: 왜 이 원석이 이 염원에 어울리는지 1~2문장으로 설명 (최대 160자)
